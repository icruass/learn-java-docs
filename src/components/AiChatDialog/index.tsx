...
const AiChatDialog: React.FC<AiChatDialogProps> = ({
  ...
  const systemPrompt = useMemo(() => {
    const { title, outline } = collectPageContext();
    return [
  ...
  // 「发现问题」：让模型基于本页知识点生成若干问题，解析成标签追加进 chips。
      ...
      await streamChat(
        [
          { role: "system", content: systemPrompt },
          {
            role: "user",
            content:
              '请基于"当前页面"的知识点与上面这段代码，提出最多 3 个有助于由浅入深理解的问题。' +
              '只返回一个 JSON 字符串数组（形如 ["问题1","问题2"]），数组长度不超过 3，不要代码块、不要任何多余文字。',
          },
        ],
        (d) => {
          buf += d;
  ...
  }, [discovering, loading, systemPrompt]);
