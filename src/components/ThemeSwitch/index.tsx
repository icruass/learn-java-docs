...
export interface ThemeSwitchProps {
  /**
   * 受控选中态（选中 = 暗色）。
   * 不传时组件自动绑定全局主题（useTheme），开箱即用。
   */
  checked?: boolean;
  /** 受控模式下的变更回调；不传时走全局 toggleTheme */
  onChange?: (checked: boolean) => void;

  // ---- 以下为原本写死、现在开放为 prop 的视觉参数（映射到 CSS 变量） ----
  /** 整体尺寸（--toggle-size），默认 '30px' */
  size?: string;
  /** 亮色模式容器背景（--container-light-bg） */
  lightBg?: string;
  /** 暗色模式容器背景（--container-night-bg） */
  nightBg?: string;
  /** 太阳颜色（--sun-bg） */
  sunColor?: string;
  /** 月亮颜色（--moon-bg） */
  ...
  /** 云朵颜色（--clouds-color） */
  cloudsColor?: string;
  /** 容器圆角（--container-radius），设 0 可得方形「Minecraft」风格 */
  radius?: string;
...
/**
 * 暗色/亮色 主题切换开关（Uiverse by Galahhad，封装版）。
 *
 * - 默认与全局主题联动：选中 = 暗色，未选中 = 亮色。
 * - 所有原本写死的尺寸/颜色都开放为 prop，通过 CSS 变量覆盖。
 *
 * <ThemeSwitch />                       // 直接联动全局主题
 * <ThemeSwitch size="24px" radius="0" /> // 自定义尺寸/方形
 */
